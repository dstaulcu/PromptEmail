...
export class PromptManager {
    ...
    async loadPrompts() {
        ...
        try {
            ...
            console.info('[INFO] - Prompt templates loaded successfully');
            return this.prompts;
        } catch (error) {
            console.error('[ERROR] - Failed to load prompt templates:', error);
    ...
    async getPrompt(category, type = 'user_prompt') {
        ...
        if (!this.prompts || !this.prompts[category] || !this.prompts[category][type]) {
            console.warn(`[WARN] - Prompt not found: ${category}.${type}, using fallback`);
