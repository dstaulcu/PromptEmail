...
export class Logger {
    ...
    async initializeTelemetryConfig() {
        try {
            if (this.debugEnabled) console.debug('[VERBOSE] - Loading telemetry configuration...');
    ...
    logEvent(eventType, data = {}, level = 'Information', contextEmail = null) {
        if (!this.isEnabled) {
            if (this.debugEnabled) console.debug('[VERBOSE] - Logging disabled:', eventType, data);
            return;
        }
            ...
            if (this.telemetryConfig?.telemetry?.enabled) {
                if (this.telemetryConfig.telemetry.provider === 'api_gateway') {
                    ...
                    this.logToApiGateway(eventType, logEntry, level).catch(error => {
                        if (this.debugEnabled) console.debug('[VERBOSE] - Telemetry logging error (non-blocking):', error.message);
                    });
                }
            }

        } catch (error) {
            console.error('[ERROR] - Failed to log event (non-blocking):', error);
        }
    }
    ...
    getUserContext(contextEmail) {
        ...
        try {
            ...
            if (this.debugEnabled) console.debug('[VERBOSE] - Could not retrieve Office user profile:', error);
        }
    ...
    getClientContext() {
        ...
        try {
            ...
            if (this.debugEnabled) console.debug('[VERBOSE] - Error getting client context:', error);
    ...
    getEnvironmentContext() {
        ...
        try {
            ...
            if (this.debugEnabled) console.debug('[VERBOSE] - Error getting environment context:', error);
    ...
    getOfficeDiagnostics() {
        ...
        try {
            ...
            if (this.debugEnabled) console.debug('[VERBOSE] - Error getting Office diagnostics:', error);
        ...
        try {
            ...
            if (this.apiGatewayQueue.length >= apiGatewayConfig.batchSize) {
                ...
                this.flushApiGatewayQueue().catch(error => {
                    if (this.debugEnabled) console.debug('[VERBOSE] - Auto-flush error (non-blocking):', error.message);
                });
            }
        } catch (error) {
        ...
        (async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    if (this.debugEnabled) console.debug('[VERBOSE] - API Gateway response:', result);
                } else {
                    ...
                    if (isTransient) {
                        ...
                        if (this.apiGatewayRetryCount <= apiGatewayConfig.retryAttempts) {
                            console.warn(`[WARN] - Transient error (${status}). Retrying in ${backoff}ms (attempt ${this.apiGatewayRetryCount}/${apiGatewayConfig.retryAttempts})`);
                            setTimeout(() => {
                ...
                if (isNetworkError) {
                    if (!this.apiGatewayConnectionError) {
                        ...
                        console.warn(`[WARN] - API Gateway ${errorType} error, events will be queued`);
                    ...
                    if (this.apiGatewayRetryCount <= apiGatewayConfig.retryAttempts) {
                        ...
                        console.warn(`[WARN] - ${errorType} error. Retrying in ${backoff}ms (attempt ${this.apiGatewayRetryCount}/${apiGatewayConfig.retryAttempts})`);
                        setTimeout(() => {
                            this.apiGatewayQueue.unshift(...events);
                            this.flushApiGatewayQueue();
                        }, backoff);
                        ...
                        const errorType = isTimeoutError ? 'timeout' : 'network';
                        console.error(`[ERROR] - Max API Gateway retry attempts reached (${errorType} error), dropping events`);
                        this.apiGatewayRetryCount = 0;
                    }
                } else {
            ...
            console.error('[ERROR] - Fire-and-forget telemetry error:', error);
        });
    }
    ...
    stopApiGatewayAutoFlush() {
        if (this.apiGatewayFlushInterval) {
            ...
            this.apiGatewayFlushInterval = null;
            if (this.debugEnabled) console.debug('[VERBOSE] - Stopped logging auto-flush');
        }
    }
